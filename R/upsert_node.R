...
upsert_node <- function(identifier, data, db_file) {
  ...
  current_data <- jsonlite::fromJSON(simplegraphdb::atomic(db_file, simplegraphdb::find_node(identifier))$body)
  updated_data <- c(current_data, data)
  updated_data <- simplegraphdb::set_id(identifier, jsonlite::toJSON(updated_data))
